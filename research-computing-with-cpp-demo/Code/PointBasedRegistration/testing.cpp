...
// Complete unit test. Point 7 of the instructions.
TEST_CASE("Testing that the normal workflow works", "[All]") { 
	...
	std::string fixedPath("fixed.txt");
	std::string movingPath("moving.txt");
	std::string matrix4x4Path("matrix.4x4");
	try {
		...
		REQUIRE(homTransformMatrix.isApprox(mat, 0.0001));
	} 
	catch (const std::exception &e) {
		std::cerr << e.what() << std::endl;	
	}
